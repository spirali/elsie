...
@check("imagefrag", expect_count=6, cairo=False)
...
@check("nofrag_showbegin", expect_count=3, cairo=False)
...
@check("image-substeps1", expect_count=3, cairo=False)
...
@check("image-substeps2", expect_count=3, cairo=False)
def test_image_substeps_show2(test_env):
    ...
    test_env.check_svg("image-substeps2", 3)


@check("image-source", cairo=True)
